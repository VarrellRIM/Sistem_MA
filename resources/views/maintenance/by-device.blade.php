...
<div class="d-flex align-items-center justify-content-between mb-3">
    <div>
        ...
        <span class="text-muted">{{ $device->brand }} {{ $device->model }} — <span class="badge bg-secondary">{{ $device->asset_code }}</span></span>
    </div>
    <div class="d-flex gap-2">
        <a href="{{ route('devices.show', $device) }}" class="btn btn-outline-secondary btn-sm">
            <i class="bi bi-pc-display me-1"></i>Detail Perangkat
        </a>
...
<div class="card table-card shadow-sm">
    <div class="table-responsive">
        <table class="table table-hover mb-0">
            <thead><tr>
                <th>Tanggal</th><th>Tipe</th><th>Deskripsi</th>
                <th>Sparepart</th><th>Teknisi</th><th>Biaya</th><th>Next Maintenance</th>
            </tr></thead>
            <tbody>
            @forelse($logs as $log)
                <tr>
            ...
            @empty
                <tr><td colspan="7" class="text-center text-muted py-4">Belum ada riwayat maintenance untuk perangkat ini.</td></tr>
            @endforelse
            </tbody>
        </table>
    </div>
